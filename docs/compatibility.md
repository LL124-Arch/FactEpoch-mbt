...
Committed JSON fixtures, their strict fixed-profile Python oracle, generated MoonBit fixture tests, generated MoonBit interfaces, and release audit commands are the compatibility evidence. The oracle rejects unknown fields and metadata drift, validates every input/expected case, and byte-compares its generated test file under `--check`. Live model output and benchmark speed are not compatibility contracts.
